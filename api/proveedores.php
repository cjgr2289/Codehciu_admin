...
function actualizarProveedor($pdo) {
    ...
    // Verificar que el ID o RIF no estén en uso por otro proveedor
    $check = $pdo->prepare("
        SELECT id FROM proveedores 
        WHERE (id = :id OR ci_rif = :ci_rif) 
        AND id != :id_actual
    ");
    $check->execute([
        ':id' => $input['id'],
    ...
    if ($check->fetch()) {
        echo json_encode(['success' => false, 'message' => 'Ya existe otro proveedor con este ID o RIF']);
        return;
    }
