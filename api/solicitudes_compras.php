 ...
 * MODIFICADO: Código incluye ID del proyecto
 */
...
/**
 * Genera preview del código (incluye el proyecto si se pasa)
 */
function generarCodigoPreview($pdo, $proyecto_id = 0) {
    ...
    if ($proyecto_id) {
        $prefijo = $subproceso_origen . '-' . $subproceso_destino . '-' . $tipo_doc . '-' . $proyecto_id . '-' . $anio . '-';
        ...
        // Fallback (sin proyecto) - no debería ocurrir
...
/**
 * Genera código de solicitud con formato: CMP-CGE-SOL-{proyecto_id}-{año}-{secuencia}
 * La secuencia es por proyecto y año.
 */
